        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 col-sm-12 ol-lg-12">
                    {!! Form::open(['method' => 'post', 'class' => 'form-cart', 'route' => 'cart.store']) !!}
                            ...
                            @if (Session::has('cart'))
                                @foreach($product_cart as $product)
                                    <tr id='delete-item{{ $product['item']['id'] }}'>
                                        ...
                                        <td class="product-price"><span class="amount">@lang('layouts.cart.dollar'){{ $product['item']->newPrice() }}</span></td>
                                        <td class="product-quantity"><input type="number" value="{{ $product['quantity'] }}"  name="number_quantity{{ $product['item']['id'] }}" class="number_quantity" id="{{ $product['item']->id }}" min="0" max="{{ $product['item']['quantity'] }}"/></td>
                        ...
                        <ul class="cart__btn__list d-flex flex-wrap flex-md-nowrap flex-lg-nowrap justify-content-between">
                            <li></li>
                            <li></li>
                            <li><input type="submit" value="{{@trans('layouts.footer.checkout')}}"/></li>
                        </ul>
                    </div>
                    {!! Form::close() !!}
                </div>
